...
<template>
  <div class="mg_header_wrap">
    <div class="mg_header flex_between">
      <div class="logo">
        <router-link :to="{path: '/'}">
          <img style="display: block; height: 33px" :src="result.logo" alt="">
        </router-link>
      </div>
      <div class="header_nav flex_row">
        <div class="item_nav">
          ...
          <div class="first_type">
            <router-link :to="{name: 'type', query: {ftId: item.t_id}}" v-for='(item, index) in result.sheji' :key="index">
        ...
        <div class="item_nav">
          <a href="javascript:;" class="nav_title">
            <span>鉴赏提升</span>
            <img class="nav_bg" v-if='result.video.length > 0' src="../../assets/images/icon_header_select.png" alt="">
            <img class="nav_bg" v-if='result.video.length == 0' src="../../assets/images/icon_header_select1.png" alt="">
          </a>
          <div class="first_type">
            <router-link :to='{name: "teachingVideo", query: {id: item.t_id, title: item.name}}' v-for='(item, index) in result.video' :key="index">
              <img class="icon_first" :src="item.icon" alt="">
              <div>{{item.name}}</div>
            </router-link>
          </div>
         ...
         <div class="item_nav">
          <a href="http://tutu.meigongpuzi.cn/#/index" class="nav_title" target="_blank">
            <span>在线设计</span>
            <img class="nav_bg" v-if='result.video.length > 0' style="height: 56px" src="../../assets/images/icon_header_select1.png" alt="">
            <img class="nav_bg" v-if='result.video.length == 0' src="../../assets/images/icon_header_select1.png" alt="">
          </a>
        </div>
        <div class="item_nav">
          <a href="javascript:;" class="nav_title">
            <span>充值中心</span>
            <img class="nav_bg" v-if='Object.keys(result.vip).length > 0' src="../../assets/images/icon_header_select.png" alt="">
          </a>
          <div class="first_type">
            <router-link :to='{ name: "rechargeVip"}' v-for='(item, index) in result.vip' :key="index">
              <img class="icon_first" :src="item.icon" alt="">
              <div>{{item.name}}</div>
            </router-link>
          </div>
        ...
        <div class="right">
          <div class="left" v-if="!userInfo.token">
            <div class="item_tool left">
              <a href="javascript:;" class="tool_btn btn_login" @click="toLogin()">请登录</a>
              <!-- <router-link :to="{name: 'login'}" class="tool_btn btn_login">请登录</router-link> -->
            </div>
            <div class="item_tool left">
              <a href="javascript:;" class="tool_btn" @click="toLogin()">免费注册</a>
              ...
              <div class="user_hover">
                <ul>
                  <li class="user_name" style="border-bottom: 1px solid #E5E5E5;">
                    <router-link :to="{name: 'user', query: {active: 'info'}}" class="tx_ov">{{userInfo.nickname}}</router-link>
                  </li>
